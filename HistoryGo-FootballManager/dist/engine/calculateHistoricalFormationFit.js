...
export function calculateHistoricalRoleCoverage(input) {
    ...
    const preferredMatched = preferredTypes.filter((type) => assignmentMaps.some((entry) => entry.hgIds.has(type)));
    const misusedMatched = misusedTypes.filter((type) => assignmentMaps.some((entry) => entry.hgIds.has(type)));
...
function calculateMetricAlignment(baseMetrics, formationBaseline) {
    ...
    const diffs = HISTORICAL_METRIC_KEYS.map((key) => Math.abs(num(baseMetrics?.[key] ?? 50) - num(formationBaseline[key])));
...
export function calculateHistoricalFormationFit(input) {
    ...
    let historicalScore = roleCoverage.coverageScore * 0.55 +
        metricAlignment * 0.3 +
        60 * 0.15;
    ...
    const roleRequirements = asArray(formation?.roleRequirements);
    const requiresLibero = roleRequirements.some((id) => LIBERO_ROLE_TYPES.includes(id)) ||
    ...
    if (roleCoverage.requiredMissing.length > 0) {
        issues.push(roleCoverage.requiredMissing.length === 1
...
export function buildHistoricalFormationReport(input) {
    ...
    if ((difficulty === "high" || difficulty === "very_high") && (roleCoverage.coverageScore ?? 60) < 55) {
        issues.push(`${formationName} er ikke en svak formasjon; laget får bare ikke frem nøkkelprinsippene den krever ennå.`);
    }
    return { strengths, issues, notes };
}
