...
function getKeyReasons(reasons, limit) {
    return [...reasons]
        .sort((a, b) => Math.abs(b.impact) - Math.abs(a.impact))
        .slice(0, limit)
        .map((reason) => reason.label);
}
function getReportLevel(evaluation) {
...
function getMetricSeverity(score) {
    if (score >= GOOD_SCORE) {
        return "positive";
    }
    if (score < LOW_SCORE) {
...
function getPlayerFitStatus(finalFit) {
...
function toPlayerFitReport(result) {
    return {
...
function buildBalanceOverview(teamBalance) {
    const items = [
        {
...
function buildOverallSummary(evaluation, level, averageRoleFit) {
...
function buildKeyStrengths(evaluation, averageRoleFit) {
...
function buildKeyProblems(evaluation) {
    return evaluation.issues.map((issue) => issue.label);
}
function addAdvice(advice, code, priority, label, rationale) {
    advice.push({
        code,
        priority,
        label,
...
function buildCoachAdvice(evaluation, averageRoleFit) {
    const advice = [];
    const balance = evaluation.teamBalance;
    const weakestFit = evaluation.worstFits[0];
    if (!evaluation.isComplete) {
        addAdvice(advice, "complete_lineup", 1, "Fullfør startelleveren før du vurderer finjusteringer.", "Motoren trenger elleve gyldige spiller-/rolleplasseringer for at lagbalansen skal bli pålitelig.");
    }
    if (averageRoleFit > 0 && averageRoleFit < GOOD_FIT) {
        addAdvice(advice, "improve_role_fit", 1, "Se først på rollebruken.", "Samlet rollefit er lavere enn ønskelig. Det tyder på at flere spillere brukes i roller som ikke får frem styrkene deres.");
    }
    if (weakestFit && weakestFit.finalFit < USABLE_FIT) {
        addAdvice(advice, "fix_worst_fit", 1, `Vurder rollen til ${weakestFit.playerId}.`, `Denne spilleren har laveste registrerte rollefit (${weakestFit.finalFit}) i oppsettet.`);
    }
    if (balance.defensiveBalance < LOW_SCORE) {
        addAdvice(advice, "strengthen_defensive_balance", 1, "Legg inn tydeligere sikring.", "Forsvarsbalansen er svak. Laget trenger bedre dekning, lavere risiko eller en tydeligere balanse-/sikringsrolle.");
    }
    if (balance.riskBalance < LOW_SCORE) {
        addAdvice(advice, "reduce_structural_risk", 1, "Reduser samlet risiko i rollevalgene.", "Risikobalansen er svak. For mange aggressive roller samtidig kan gjøre laget sårbart i overgangene.");
    }
    if (balance.pressingCoherence < LOW_SCORE) {
        addAdvice(advice, "align_pressing", 2, "Samkjør presset.", "Presset henger dårlig sammen. Enten trenger laget flere pressroller, eller så bør presshøyden senkes.");
    }
    if (balance.widthBalance < LOW_SCORE) {
        addAdvice(advice, "fix_width_structure", 2, "Rydd breddestrukturen.", "Bredden fungerer svakt. Laget bør ha tydeligere roller som holder bredde eller en taktikk som passer smalere rollevalg.");
    }
    if (balance.midfieldControl < LOW_SCORE) {
        addAdvice(advice, "improve_midfield_control", 2, "Styrk midtbanekontrollen.", "Midtbanen gir for lite kontroll. Vurder tempo-kontrollør, bedre oppbyggingsrolle eller tydeligere balanse sentralt.");
    }
    if (advice.length === 0) {
        addAdvice(advice, "keep_structure", 3, "Behold grunnstrukturen.", "Oppsettet har ingen tydelige kritiske problemer i denne rapporten. Neste steg bør være finjustering, ikke stor ombygging.");
    }
    return advice;
}
export function createTeamSetupReport(evaluation) {
