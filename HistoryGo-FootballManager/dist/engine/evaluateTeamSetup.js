...
import { calculateRoleRelationships, } from "./calculateRoleRelationships.js";
const LOW_SCORE = 60;
...
function resolveAssignments(team, tactic, roles) {
...
function calculateAverageRoleFit(roleFitResults) {
    return average(roleFitResults.map((result) => result.finalFit), 0);
}
function getBestFits(roleFitResults) {
    return [...roleFitResults]
        .sort((a, b) => b.finalFit - a.finalFit)
        .slice(0, 3);
}
function getWorstFits(roleFitResults) {
    return [...roleFitResults]
        .sort((a, b) => a.finalFit - b.finalFit)
        .slice(0, 3);
}
function addIssue(issues, code, severity, label) {
    issues.push({
        code,
        severity,
        label,
...
function buildStrengths(teamBalance, averageRoleFit) {
    const strengths = [];
...
function buildIssues(missingAssignments, teamBalance, averageRoleFit) {
    const issues = [];
    if (missingAssignments.length > 0) {
        addIssue(issues, "missing_assignments", "critical", "Startelleveren har manglende eller ugyldige rolleplasseringer.");
    }
    if (averageRoleFit > 0 && averageRoleFit < 70) {
        addIssue(issues, "weak_average_role_fit", "warning", "Laget har svak samlet rollefit.");
    }
    if (teamBalance.finalBalance < LIMITED_SCORE) {
        addIssue(issues, "weak_team_balance", "warning", "Lagbalansen har tydelige strukturelle problemer.");
    }
    if (teamBalance.pressingCoherence < LOW_SCORE) {
        addIssue(issues, "weak_pressing_coherence", "warning", "Presset henger dårlig sammen.");
    }
    if (teamBalance.widthBalance < LOW_SCORE) {
        addIssue(issues, "weak_width_balance", "warning", "Breddestrukturen er svak.");
    }
    if (teamBalance.defensiveBalance < LOW_SCORE) {
        addIssue(issues, "weak_defensive_balance", "warning", "Forsvarsbalansen er sårbar.");
    }
    if (teamBalance.riskBalance < LOW_SCORE) {
        addIssue(issues, "weak_risk_balance", "warning", "Risikonivået er dårlig balansert.");
    }
    return issues;
}
function buildSummary(isComplete, setupScore, issues) {
    if (!isComplete) {
...
export function evaluateTeamSetup(input) {
    const { team, tactic, roles } = input;
    const { resolvedAssignments, missingAssignments } = resolveAssignments(team, tactic, roles);
    const roleFitResults = resolvedAssignments.map(({ player, role, assignment }) => calculateRoleFit(player, role, assignment, tactic));
    ...
    const relationshipAssignments = resolvedAssignments.map(({ player, role, assignment }) => ({
        roleId: role.id,
        position: assignment.position,
    ...
    const isComplete = totalAssignmentCount === 11 &&
        validAssignmentCount === 11 &&
        missingAssignments.length === 0;
    const setupScore = clampScore(teamBalance.finalBalance * 0.65 +
        averageRoleFit * 0.35 -
