...
function suggestionsFromRisks(risks) {
    return unique(risks
 ...
 */
export function deriveFormationStyleTokens(profile) {
    const tokens = [];
    const p = profile;
    if (p.pressHeight === "high")
        tokens.push("high_press");
    if (p.pressHeight === "high" && p.pressingScheme === "man_oriented")
        tokens.push("aggressive_man_press");
    if (p.defensiveLine === "deep" || (p.pressHeight === "low" && p.restDefence === "strong"))
        tokens.push("deep_low_block");
    if (p.possession === "patient")
        tokens.push("possession_positional");
    if (p.possession === "patient" && p.defensiveLine === "high")
        tokens.push("build_from_back");
    if (p.possession === "patient" && p.width === "wide")
        tokens.push("switching_play");
    if (p.possession === "direct" && p.transition === "high")
        tokens.push("direct_counter");
    if (p.transition === "high" && p.tempo === "high")
        tokens.push("fast_transition");
    if (p.transition === "high" && (p.possession === "direct" || p.tempo === "high"))
        tokens.push("fast_runners_in_behind");
    if (p.width === "wide")
        tokens.push("wide_overload");
    return unique(tokens);
}
/**
 * Form-baserte tokens utledet fra baseShape (f.eks. "5-3-2", "4-4-2").
 */
export function deriveFormationShapeTokens(baseShape) {
    ...
    if (shape === "4-4-2")
        tokens.push("narrow_442");
    if (shape === "5-3-2")
        tokens.push("compact_532");
    if (back === "3")
        tokens.push("three_at_back");
    if (front === "2")
        tokens.push("two_striker_press");
    return unique(tokens);
}
function embodiedTokens(side) {
    return unique([
 ...
 */
export function evaluateFormationMatchup(you, opponent) {
 ...
 */
export function evaluateFormationVsOpponentStyles(you, opponentStyles, opponentName) {
    const yourTokens = embodiedTokens(you);
    const opponentTokens = unique((Array.isArray(opponentStyles) ? opponentStyles : []).filter((token) => typeof token === "string" && token.length > 0));
    const advantages = [];
    const risks = [];
    for (const token of intersect(you.strongAgainst, opponentTokens)) {
        advantages.push({
