...
function addWeakPoint(weakPoints, weakPoint) {
    weakPoints.push(weakPoint);
}
function getAverageRoleFit(roleFitResults) {
    return average(roleFitResults.map((result) => result.finalFit), 0);
}
function getWorstPlayerIds(roleFitResults, limit = 3) {
    return [...roleFitResults]
        .sort((a, b) => a.finalFit - b.finalFit)
        .slice(0, limit)
        .map((result) => result.playerId);
}
function analyzeCompletion(evaluation, weakPoints) {
...
function analyzeRoleFit(evaluation, weakPoints) {
...
function analyzeTeamBalance(evaluation, weakPoints) {
...
function sortWeakPoints(weakPoints) {
    const severityRank = {
        high: 3,
        medium: 2,
        low: 1,
...
function buildSummary(weakPoints) {
...
export function analyzeWeakPoints(evaluation) {
    const weakPoints = [];
