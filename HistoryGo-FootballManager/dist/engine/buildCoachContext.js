...
const PROFILE_DIMS = Object.keys(PROFILE_BASELINE);
...
export function calculateStaffEffectProfile(input) {
    ...
    const rolesById = new Map(roles.filter((role) => Boolean(role && role.id)).map((role) => [role.id, role]));
    const accumulated = {};
...
export function calculateFormationFamiliarity(input) {
    ...
    const tokens = input.expertiseTokens instanceof Set ? input.expertiseTokens : new Set();
...
export function calculateCoachUnderstanding(input) {
    const profile = input.effectProfile;
    const base = (num(profile?.tacticalLearningSpeed) + num(profile?.formationFamiliarity) + num(profile?.roleFitClarity)) / 3;
    ...
    const hasAnalyticalEye = asArray(input.activeStaff).some((member) => member && (member.category === "analyst" || member.category === "tactical_coach"));
    if (hasAnalyticalEye) {
        understanding += 4;
    }
...
export function calculateTacticalLearningSpeed(input) {
    ...
    const learningStaff = asArray(input.activeStaff).filter((member) => member && member.category !== null && LEARNING_CATEGORIES.includes(member.category)).length;
...
export function calculateFormationDifficultyRelief(input) {
    ...
    const support = (num(input.coachUnderstanding) + num(input.formationFamiliarity) + num(input.tacticalLearningSpeed)) / 3;
...
export function buildCoachContext(input) {
    ...
    const historicalFitSupport = clamp(Math.round((coachUnderstanding - GENERAL_BASELINE) * 0.5 + themeMatches.length * 8), 0, 40);
    const difficulty = input.formation?.tacticalDifficulty;
