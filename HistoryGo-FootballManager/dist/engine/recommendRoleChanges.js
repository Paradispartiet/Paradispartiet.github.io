...
function findAlternativeRoles(roles, assignment, currentRoleId) {
    return roles.filter((role) => role.id !== currentRoleId &&
        role.allowedPositions.includes(assignment.position));
}
function toCandidate(role, fit, currentFit) {
    return {
        roleId: role.id,
        roleName: role.name,
        fit: fit.finalFit,
...
function getStatus(improvement) {
...
function buildCannotEvaluateRecommendation(assignment, player, currentRole) {
    const missingParts = [];
    if (!player) {
...
function buildRecommendationForAssignment(team, tactic, roles, assignment) {
...
export function recommendRoleChanges(team, tactic, roles) {
    const recommendations = tactic.roleAssignments.map((assignment) => buildRecommendationForAssignment(team, tactic, roles, assignment));
