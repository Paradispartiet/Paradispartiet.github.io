...
const DEFAULT_ATTRIBUTE_VALUE = 76;
const STRONG_ATTRIBUTE_VALUE = 88;
const ELITE_ATTRIBUTE_VALUE = 92;
...
function toPositionGroup(positionGroup) {
    if (positionGroup === "keeper")
        return "GK";
    if (positionGroup === "defender")
        return "DEF";
    if (positionGroup === "fullback")
        return "DEF";
    if (positionGroup === "midfield")
        return "MID";
    if (positionGroup === "attacking_midfield")
        return "MID";
    if (positionGroup === "wing")
        return "ATT";
    if (positionGroup === "striker")
        return "ATT";
    return "MID";
}
...
function boostAttribute(attributes, attribute, value) {
    attributes[attribute] = Math.max(attributes[attribute], value);
}
function applyLegacyTagToAttributes(attributes, tag) {
    if (tag === "dribbling" ||
        tag === "one_vs_one" ||
        tag === "isolation" ||
        tag === "cut_inside" ||
...
function attributesFromLegacyPlayer(player) {
...
function traitsFromLegacyPlayer(player) {
    const tags = new Set([
        ...(player.archetypes ?? []),
        ...(player.strengths ?? []),
        ...(player.needs ?? []),
    ...
    const traits = [];
    if (tags.has("dribbler") || tags.has("one_vs_one"))
        traits.push("elite_dribbler");
    if (tags.has("creator") || tags.has("final_pass"))
        traits.push("through_ball_specialist");
    if (tags.has("tempo_setter") || tags.has("controller"))
        traits.push("tempo_controller");
    if (tags.has("runner") || tags.has("late_runs"))
        traits.push("late_box_runner");
    if (tags.has("box_finishing") || tags.has("box_presence"))
        traits.push("box_finisher");
    if (tags.has("pressing_runs") || tags.has("pressing"))
        traits.push("pressing_forward");
    if (tags.has("target") || tags.has("aerial"))
        traits.push("target_player");
    if (tags.has("depth_threat") || tags.has("space_behind"))
        traits.push("space_attacker");
    if (tags.has("wide_creator") || tags.has("wide_support"))
        traits.push("wide_creator");
    if (tags.has("inverted") || tags.has("cut_inside"))
        traits.push("inverted_creator");
    if (tags.has("line_breaker") || tags.has("line_breaking_pass"))
        traits.push("line_breaker");
    if (tags.has("balance") || tags.has("screen"))
        traits.push("ball_winner");
    if (tags.has("passer") || tags.has("deep_playmaker"))
        traits.push("deep_playmaker");
    if (tags.has("defensive_presence"))
        traits.push("defensive_leader");
    if (tags.has("duel"))
        traits.push("aggressive_duelist");
    if (tags.has("covering") || tags.has("defensive_reading"))
        traits.push("cover_defender");
    if (tags.has("sweeper_keeper"))
        traits.push("sweeper_keeper");
    return unique(traits);
}
function tacticalNeedsFromLegacyPlayer(player) {
    const needs = new Set([
        ...(player.needs ?? []),
        ...(player.likesTactics ?? []),
    ]);
    return {
        needsWidth: needs.has("wide_space") ||
            needs.has("wide_attack") ||
            needs.has("wide_lane") ||
            needs.has("attacking_width"),
        needsFreedom: needs.has("creative_freedom") ||
            needs.has("space_between_lines"),
        needsRunnersAround: needs.has("runners_around") ||
            needs.has("runners_ahead") ||
            needs.has("third_man_runs"),
        needsLowBlockBehind: needs.has("compact_defense") ||
            needs.has("medium_or_low_line") ||
            needs.has("protected_box"),
        needsHighPressTeam: needs.has("high_press") ||
            needs.has("pressing_triggers"),
        needsBallDominance: needs.has("possession") ||
            needs.has("structured_build_up") ||
            needs.has("passing_options") ||
            needs.has("central_control"),
        needsDirectService: needs.has("direct_play") ||
            needs.has("direct_counter") ||
            needs.has("early_through_balls"),
        needsCombinationPlayers: needs.has("central_combinations") ||
            needs.has("close_support") ||
            needs.has("combination_play"),
        needsDefensiveCover: needs.has("cover_from_midfield") ||
            needs.has("cover_behind") ||
            needs.has("rest_defense") ||
            needs.has("stable_rest_defense"),
    };
}
function requiredAttributesFromLegacyRole(role) {
    const attributes = emptyAttributes();
    const required = {};
...
function preferredTraitsFromLegacyRole(role) {
    const fakePlayer = {
        id: role.id,
        name: role.name,
        classHeight: 90,
...
function badTraitsFromLegacyRole(role) {
    const tags = new Set([
        ...(role.badFor ?? []),
        ...(role.tacticalDislikes ?? []),
    ]);
    const traits = [];
    if (tags.has("back_to_goal_striker"))
        traits.push("target_player");
    if (tags.has("chaos"))
        traits.push("risk_taker");
    if (tags.has("constant_forward_runs"))
        traits.push("late_box_runner");
    if (tags.has("deep_block_only"))
        traits.push("sweeper_keeper");
    if (tags.has("long_ball_only"))
        traits.push("tempo_controller");
    return unique(traits);
}
function tacticalFunctionsFromLegacyRole(role) {
    const tags = new Set([
        ...(role.requires ?? []),
        ...(role.goodWith ?? []),
        ...(role.tacticalLikes ?? []),
    ...
    const functions = [];
    if (tags.has("chance_creation") || tags.has("creativity") || tags.has("vision"))
        functions.push("creator");
    if (tags.has("off_ball_runs") || tags.has("space_behind") || tags.has("late_runs"))
        functions.push("runner");
    if (tags.has("finishing") || tags.has("box_finishing") || tags.has("box_service"))
        functions.push("finisher");
    if (tags.has("pressing") || tags.has("high_press"))
        functions.push("press_trigger");
    if (tags.has("tempo_control") || tags.has("central_control"))
        functions.push("tempo_controller");
    if (tags.has("protected_zone") || tags.has("compact_midfield"))
        functions.push("space_holder");
    if (tags.has("duels") || tags.has("heading"))
        functions.push("duel_winner");
    if (tags.has("defensive_balance") || tags.has("rest_defense"))
        functions.push("balance_player");
    if (tags.has("covering") || tags.has("box_protection"))
        functions.push("cover_player");
    if (tags.has("structured_build_up") || tags.has("build_up"))
        functions.push("build_up_player");
    if (tags.has("wide_attack") || tags.has("wide_support") || tags.has("wide_lane"))
        functions.push("wide_progressor");
    return unique(functions.length > 0 ? functions : ["balance_player"]);
}
function adaptPressing(value) {
    if (value === "low")
        return "low";
    if (value === "medium_low")
        return "medium";
...
function adaptTempo(value) {
    if (value === "fast")
        return "fast";
    if (value === "controlled")
        return "slow";
    if (value === "direct_when_possible")
        return "fast";
    return "balanced";
}
function adaptWidth(value) {
    if (value === "wide")
        return "wide";
    if (value === "medium_wide")
        return "wide";
    if (value === "compact")
        return "narrow";
    if (value === "medium")
...
function tacticPrinciplesFromLegacyTactic(tactic) {
...
export function adaptLegacyPlayers(players) {
...
export function adaptLegacyRoles(roles) {
...
export function adaptLegacyTeam(input) {
    return {
        id: input.teamId,
        name: input.teamName,
...
export function adaptLegacyTactic(input) {
    const roleAssignments = input.formation.slots
        .map((slot) => {
