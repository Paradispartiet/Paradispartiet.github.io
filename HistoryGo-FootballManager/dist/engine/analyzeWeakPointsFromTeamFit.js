...
export function analyzeWeakPointsFromTeamFit(teamFit) {
    const weakPoints = [];
    // Fullføring.
    ...
    const misused = teamFit.assignments.filter((assignment) => assignment.isComplete && assignment.fit?.status === "feilbrukt");
    if (misused.length > 0) {
        weakPoints.push({
            ...
            relatedPlayerIds: misused.map((assignment) => assignment.player?.id).filter((id) => Boolean(id)),
        });
    }
    // Duplikatspillere.
