...
(function (global) {
  ...
  function loadPlaceLearningSurface() {
    ...
    const script=document.createElement("script"); script.src="js/ui/place-learning-surface.js"; script.defer=true; document.body.appendChild(script);
  }

  function loadRoundContentGuard() {
    if (global.HGPlaceCardRoundContentGuard || global.__HG_PLACE_CARD_ROUND_CONTENT_GUARD_REQUESTED__ || document.querySelector('script[src="js/ui/place-card-round-content-guard.js"]')) return;
  ...
  function renderStatus(place) {
    ...
    let row=metaEl.querySelector(`[${ROW_ATTR}]`); if(!row){row=document.createElement("div");row.setAttribute(ROW_ATTR,"1");row.className="pc-progress-status-line";metaEl.appendChild(row);}
    row.textContent=`Status: ${parts.join(" · ")}`; row.dataset.status=safeText(summary.status||"unknown"); row.dataset.visited=summary.visited?"1":"0"; row.dataset.quizCompleted=summary.quizCompleted?"1":"0"; row.dataset.favorite=summary.favorite?"1":"0";
  }
  function loadNearbyStatusSurface() { if(global.__HG_NEARBY_STATUS_SURFACE_SCRIPT_REQUESTED__||global.HGNearbyStatusSurface||document.querySelector('script[src="js/ui/nearby-status-surface.js"]'))return; global.__HG_NEARBY_STATUS_SURFACE_SCRIPT_REQUESTED__=true; const script=document.createElement("script");script.src="js/ui/nearby-status-surface.js";script.defer=true;document.body.appendChild(script); }
  function loadAreaOverviewSurface() { if(global.__HG_AREA_OVERVIEW_SCRIPT_REQUESTED__||global.HGAreaOverview||document.querySelector('script[src="js/ui/area-overview.js"]'))return; global.__HG_AREA_OVERVIEW_SCRIPT_REQUESTED__=true;const script=document.createElement("script");script.src="js/ui/area-overview.js";script.defer=true;script.addEventListener("load",()=>{if(document.querySelector('script[src="js/ui/area-overview-scroll.js"]'))return;const scrollScript=document.createElement("script");scrollScript.src="js/ui/area-overview-scroll.js";scrollScript.defer=true;document.body.appendChild(scrollScript);},{once:true});document.body.appendChild(script); }
  function install() {
